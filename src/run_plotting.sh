...
if [[ $attack_type == "copaAtt1" ]]; then
    ...
    for inter_burst_time in "${inter_burst_choices[@]}"; do
        ...
        OUTPUT_PREFIX=$OUTPUT_ROOT_DIR/delay[$delay]-delay_budget[$delay_budget]-mode[$mode]-duration[$duration]-interval[$interval]
        INPUT_DIR=$INPUT_ROOT_DIR/rate[8]-delay[$delay]-mode[$mode]
        INPUT_PREFIX=rate[8]-delay[$delay]-buf_size[$size]-cca[genericcc_markovian]-delay_budget[$delay_budget]-mode[$mode]-duration[$duration]-interval[$interval]
...
elif [[ $attack_type == "copaAtt2" ]]; then
    for delay_budget in "${delay_budget_choices[@]}"; do
    for delay in "${delay_ms_choices[@]}"; do
    for size in "${buf_size_bdp_choices[@]}"; do
    for burst_duration in "${burst_duration_choices[@]}"; do
